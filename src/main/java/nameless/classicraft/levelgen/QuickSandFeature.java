...
public class QuickSandFeature extends Feature<QuickSandFeature.Configuration> {

    private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

    public QuickSandFeature(Codec<QuickSandFeature.Configuration> pCodec) {
        super(pCodec);
    }

    public boolean place(FeaturePlaceContext<QuickSandFeature.Configuration> pContext) {
        BlockPos blockpos = pContext.origin();
        WorldGenLevel worldgenlevel = pContext.level();
        RandomSource randomsource = pContext.random();
        QuickSandFeature.Configuration lakefeature$configuration = pContext.config();
        if (blockpos.getY() <= worldgenlevel.getMinBuildHeight() + 4) {
            ...
            if (blockstate1.getBlock().defaultBlockState().is(ModBlocks.QUICKSAND.get())
                    || blockstate1.getBlock().defaultBlockState().is(ModBlocks.RED_QUICKSAND.get())) {
    ...
    public static record Configuration(BlockStateProvider fluid, BlockStateProvider barrier) implements FeatureConfiguration {
        public static final Codec<QuickSandFeature.Configuration> CODEC = RecordCodecBuilder.create((p_190962_) -> {
            return p_190962_.group(BlockStateProvider.CODEC.fieldOf("quicksand").forGetter(QuickSandFeature.Configuration::fluid), BlockStateProvider.CODEC.fieldOf("barrier").forGetter(QuickSandFeature.Configuration::barrier)).apply(p_190962_, QuickSandFeature.Configuration::new);
        });
    }
}