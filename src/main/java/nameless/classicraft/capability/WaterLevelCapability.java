...
import nameless.classicraft.init.ModCriteriaTriggers;
import nameless.classicraft.init.ModEffects;
import nameless.classicraft.init.ModEnchantments;
import nameless.classicraft.network.PlayerWaterLevelMessage;
...
public class WaterLevelCapability {
    ...
    public static boolean canPlayerAddWaterExhaustionLevel(Player player) {
        ...
        return !(player instanceof FakePlayer) && !player.isCreative() && !player.isSpectator() && player.getCapability(ModCapabilities.PLAYER_WATER_LEVEL) != null && player.level.getDifficulty() != PEACEFUL;
    }
    ...
    public void addExhaustion(Player player, float add) {
        ...
        for (ItemStack stack : player.getArmorSlots()) {
            moisturizingLevel += EnchantmentHelper.getItemEnchantmentLevel(ModEnchantments.MOISTURIZING.get(), stack);
        }
        float moisturizingRate = 1.0f;
        if (moisturizingLevel == 1) moisturizingRate = 0.7f;
        if (moisturizingLevel >= 2) moisturizingRate = 0.5f;

        float finalValue = (float) ((double) add * ClassiCraftConfiguration.WATER_REDUCING_RATE.get()) * moisturizingRate;
        MobEffectInstance effect = player.getEffect(ModEffects.THIRST.get());
        if (effect != null) {
    ...
    public static void restoreUpdate(Player player) {
        if (!player.level.isClientSide()) {
            ...
            ModCriteriaTriggers.WATER_LEVEL_RESTORED_TRIGGER.trigger(serverPlayer);
            player.getCapability(ModCapabilities.PLAYER_WATER_LEVEL).ifPresent(t -> SimpleNetworkHandler.CHANNEL.send(PacketDistributor.PLAYER.with(() -> serverPlayer), new PlayerWaterLevelMessage(t.getWaterLevel(), t.getWaterSaturationLevel(), t.getWaterExhaustionLevel())));
        }
    ...
    protected static void mobEffectPunishment(Player player, int level) {
        int weAmp = ClassiCraftConfiguration.WEAKNESS_EFFECT_AMPLIFIER.get();
        int slAmp = ClassiCraftConfiguration.WEAKNESS_EFFECT_AMPLIFIER.get();
    ...
    public static class Provider implements ICapabilitySerializable<Tag> {
        ...
        public <T> LazyOptional<T> getCapability(@Nonnull Capability<T> cap, @Nullable Direction side) {
            ...
            return ModCapabilities.PLAYER_WATER_LEVEL.orEmpty(cap, handler);
        }
        ...
        public Tag serializeNBT() {
            return WaterLevelCapability.writeNBT(ModCapabilities.PLAYER_WATER_LEVEL, instance, null);
        }

        @Override
        public void deserializeNBT(Tag nbt) {
            WaterLevelCapability.readNBT(ModCapabilities.PLAYER_WATER_LEVEL, instance, null, nbt);
        }
    }
