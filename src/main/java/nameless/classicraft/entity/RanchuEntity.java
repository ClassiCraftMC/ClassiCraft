...
public class RanchuEntity extends Animal implements Bucketable {
    ...
    public static final Ingredient FOOD_ITEMS = Ingredient.of(ModItems.RICE_HUSK.get());
    ...
    public boolean isFood(ItemStack stack) {
        return stack.is(ModItems.RICE_HUSK.get());
    }
    ...
    public static boolean checkFishSpawnRules(EntityType<? extends RanchuEntity> type, LevelAccessor worldIn, MobSpawnType reason, BlockPos p_223363_3_, RandomSource randomIn) {
    ...
    public ItemStack getBucketItemStack() {
        return new ItemStack(ModItems.RANCHU_BUCKET.get());
    }
    ...
    @Nullable
    @Override
    public RanchuEntity getBreedOffspring(ServerLevel p_241840_1_, AgeableMob ranchuB) {
        RanchuEntity child = ModEntities.RANCHU_ENTITY.get().create(p_241840_1_);
        RandomSource rand = this.getRandom();
        if (ranchuB instanceof RanchuEntity) {
            // Feral + Feral
            if (this.getVariant() <= 2 && ((RanchuEntity) ranchuB).getVariant() <= 2) {
            ...
            // Fancy + Fancy
            else if (this.getVariant() > 2 && ((RanchuEntity) ranchuB).getVariant() > 2) {
                child.setVariant(rand.nextInt(RanchuEntity.MAX_VARIANTS - 3) + 3);
            }

            // Feral + Fancy
            else if (this.getVariant() <= 2 || ((RanchuEntity) ranchuB).getVariant() <= 2 && this.getVariant() > 2 || ((RanchuEntity) ranchuB).getVariant() > 2) {
                if (rand.nextBoolean()) {
                    child.setVariant(rand.nextInt(RanchuEntity.MAX_VARIANTS - 3) + 3);
                } else {
                    child.setVariant(rand.nextInt(3) + 1);
    ...
    public ItemStack getPickedResult(HitResult target) {
        return new ItemStack(ModItems.RANCHU_SPAWN_EGG.get());
    }
