...
public class StonePebbleBlock extends Block {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        final ItemStack stack = getCloneItemStack(state, hit, world, pos, player);
        giveItemToPlayer(world, player, stack);
        world.removeBlock(pos, false);
        return ActionResult.SUCCESS;
    }


    public static void giveItemToPlayer(World world, PlayerEntity player, ItemStack stack) {
        if (!stack.isEmpty() && !world.isClient) {
            final ItemEntity entity = new ItemEntity(world, player.getX(), player.getY() + 0.5, player.getZ(), stack);
