...
public class RedSandstoneHeapBlock extends AbstractPickableBlock {

    public RedSandstoneHeapBlock() {
        super(Properties.of(Material.DIRT).sound(SoundType.STONE).strength(0.15f).noCollission().noOcclusion());
    }

    @Override
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        giveItemToPlayer(level, player, ModItems.RED_SANDSTONE_PEBBLE.get());
